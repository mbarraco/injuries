...
player_id, team_id, type_id — no names. This script:
  ...
  4. Enriches players/teams beyond just a name, using data ALREADY present
     ...
     types).
  5. Writes a consolidated data/sportmonks_entities.json lookup plus
...
FIXTURES_CACHE_DIR = os.path.join(os.path.dirname(__file__), "data", "raw",
                                  "sportmonks", "fixtures")
RAW_ROOT = os.path.join(os.path.dirname(__file__), "data", "raw", "sportmonks")
ENTITIES_FILE = os.path.join(os.path.dirname(__file__), "data",
                             "sportmonks_entities.json")
